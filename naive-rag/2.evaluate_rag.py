...
from ragas.metrics import (
    Faithfulness,
    ContextRecall,
    ContextPrecision, # 'context_relevancy' no TRACe
)
...
def run_rag_pipeline_for_question(item: dict, rag_cursor, llm, embeddings_model) -> dict:
    """
    Executa o pipeline RAG (recuperação + geração) para uma única pergunta
    ...
    retrieved_context = retrieve_context_from_db(question_text, rag_cursor, embeddings_model)
...
def run_evaluation(rag_conn, rag_cursor, scraping_cursor, llm, embeddings_model, openai_api_key: str):
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        future_to_question = {
            executor.submit(run_rag_pipeline_for_question, item, rag_cursor, llm, embeddings_model): item
    ...
    print("\n--- Avaliação Completa do RAG Naive Concluída! ---")


if __name__ == "__main__":
    print("--- Iniciando Script de Avaliação do RAG Naïve ---")
    load_dotenv()
    ...
    scraping_conn = None
    try:
        print("-> Conectando ao banco de dados do RAG Naive (tcc_db)...")
        ...
        print("\n--- Conexões estabelecidas. Iniciando avaliação robusta. ---\n")
        run_evaluation(rag_conn, rag_cursor, scraping_cursor, llm, embeddings_model, openai_api_key)
